...
const BOOKMARK_WRITE_FIELDS = ['boardId', 'title', 'description'];
...
class DashboardlyDataLoader {
  ...
  updateBookmark(bookmarkData) {
    // we are not sure if this is proper validation for bookmarks
    const errors = validate.boardUpdate(bookmarkData);
    if (errors) {
      return Promise.reject({ errors: errors });
    }
